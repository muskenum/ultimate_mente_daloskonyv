...
def getFileInfo(nwcData):
	...
	print 'title,author,copyright1,copyright2,comments: ', (title,
...
def getMargins(nwcData):
	...
	temp = nwcData.read(9)
	if debug:
		print 'skipping ', binascii.hexlify(temp)
...
def getFonts(nwcData):
	...
	for i in range(12):
		...
		if debug:
			print 'Font detected', font, 'size', size, 'style ', style, \
				' typeface', typeface
...
def findStaffInfo(nwcData):
	...
	print '[staffName,groupName,endingBar,stafftype,noOfLines]', \
		[staffName, groupName, endingBar, stafftype, noOfLines]
...
def getLyrics(nwcData):
	...
	print 'blocks ', blocks, 'lyrics len', lyricsLen, 'at ', \
		nwcData.tell()
...
def getTempoVariance(data):
	...
	if data == 0 or data == 1:
		...
		temp = '_\\markup {\\small \\italic \\bold {' \
			+ tempoVariance[data] + '}}'
	return temp
...
try:
	...
	if debug:
		...
		print 'bottom ', str(bottomMargin)
		print 'left ', str(leftMargin)
		print 'right ', str(rightMargin)
...
print
print
print 'Please send all bugs and requests to zz85nus@gmail.com or visit https://github.com/zz85/nwc2ly.py/issues'
